...
def train(featype, language, mainSavedir, Resdir, best_model, ifgpu, iffinetune, datasetdir):
    Savedir = os.path.join(mainSavedir, 'label', featype)
    Resultsdir = os.path.join(Resdir, 'label', featype)
    if ifgpu == 'true':
    ...
    conf.update({'resnet_dropout': 0.3})
    ...
    train_data = MyDataSet(datasetdir, tokenizer, 'train')
    test_data = MyDataSet(datasetdir, tokenizer, 'test')
    ...
    # criterion = MultiClassFocalLossWithAlpha(alpha=alpha1)
    ...
    for epoch in trange(int(NUM_TRAIN_EPOCHS), desc='Epoch'):
        ...
        for i, data in enumerate(data_loader_train):
            indexs, texts, texts_mask, images, labels = data
            ...
            out = model(texts, texts_mask, images)
            ...
            train_label.extend(labels.cpu().data.tolist())
            train_predict.extend(torch.argmax(out, dim=1).cpu().detach().tolist())
        ...
        if epoch != 100:
            print('*'*10 + 'Test' + '*'*10)
            acc, reca, f1, loss = eval(model, device, criterion, data_loader_test, Resultsdir, epoch)
            message = "Epoch %d:  (%f,%f,%f)   loss: %f \n" % (epoch, acc, reca, f1, loss)
            log_file = os.path.join(Resultsdir, "performance.log")
            logging_func(log_file, message)
            print('Test acc:{} recall:{} f1score:{} loss:{}'.format(acc, reca, f1, loss.item()))



def eval(model, device, criterion, loader, resdir, epoch):
    model.eval()
    loss_total = 0
    predict_all = []
    labels_all = []
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            labels_all.extend(labels.cpu().data.tolist())
            predict_all.extend(torch.argmax(out, dim=1).cpu().detach().tolist())

        acc = accuracy_score(labels_all, predict_all)
        f1 = f1_score(labels_all, predict_all, average='macro')
        recall = recall_score(labels_all, predict_all, average='macro')
        np.savetxt(os.path.join(resdir, 'predict_'+ str(epoch) + '.txt'), predict_all, fmt='%d', newline=" ")
        np.savetxt(os.path.join(resdir, 'origin' + '.txt'), labels_all, fmt='%d', newline=" ")

        return acc, recall, f1, loss_total
