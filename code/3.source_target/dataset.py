...
def pad_custom_sequence(sequence):
    ...
    label_sequence1 = torch.LongTensor([b[3] for b in sequence])
    label_sequence2 = torch.LongTensor([b[4] for b in sequence])
    ...
    return id_sequence, paded_texts, paded_mask_sequence, image_sequence, label_sequence1, label_sequence2
...
class MyDataSet(Dataset):
    ...
    def __init__(self, path, tokenizer, language, dset):
        ...
        if dset == 'train':
            file = 'target_train.csv'
        elif dset == 'test':
            file = 'target_test.csv'
    ...
    def __getitem__(self, index):
        ...
        label1 = int(float(self.labels1[index]))

        label2 = int(float(self.labels2[index]))

        return index, encoded_text, img, label1, label2
