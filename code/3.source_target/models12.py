...
class Model(torch.nn.Module):
    ...
    def forward(self, texts, texts_mask, imgs, labels=None):
        ...
        # attention_out1 = self.attention1(text_feature)
        prob_vec1 = self.classifier1(torch.cat([text_feature, img_feature], dim=1))
        
        # attention_out2 = self.attention2(text_feature)
        prob_vec2 = self.classifier2(torch.cat([text_feature, img_feature], dim=1))
        # pred_labels1 = torch.argmax(prob_vec1, dim=1)
        # pred_labels2 = torch.argmax(prob_vec2, dim=1)
        # pred_labels1 = torch.softmax(prob_vec1, dim=1)
        # pred_labels2 = torch.softmax(prob_vec2, dim=1)
        

        return prob_vec1, prob_vec2
        # return pred_labels1, pred_labels2
